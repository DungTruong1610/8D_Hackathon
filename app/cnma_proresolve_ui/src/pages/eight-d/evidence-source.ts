...
function classify(raw: string): EvidenceSource {
    ...
    if (/GW_8D/i.test(trimmed) || /SAP HANA.*GW_8D/i.test(trimmed)) {
        return {
            label: 'supported by SAP HANA',
            kind: 'sap',
            detail: 'Retrieved via SAP HANA Knowledge Graph Workspace (GW_8D).',
        };
    }
    ...
    if (/^ASSUMED\s*:/i.test(trimmed)) {
        ...
        return {
            ...
            kind: 'external',
            detail: `Assumed from ${system}, a system outside SAP that is not connected here. `
                + 'No SAP record backs this line.',
        };
    }

    if (/^SAP\s*:/i.test(trimmed)) {
        const module = trimmed.replace(/^SAP\s*:\s*/i, '');
        return { label: `SAP ${module}`, kind: 'sap', detail: `Read from SAP ${module}.` };
    }
     ...
     * SAP chống lưng lại bị gắn nhãn 'suy ra', tức nói giảm đúng thứ đáng tin
     * nhất trong danh sách.
     */
    if (SAP_OBJECTS.test(trimmed)) {
        return { label: trimmed, kind: 'sap', detail: `Read from SAP — ${trimmed}.` };
    }
...
const SAP_NOTIFICATION: EvidenceSource = {
    label: 'SAP QM notification',
    kind: 'sap',
    detail: 'Quality notification in SAP QM.',
};
...
const BY_PREFIX: Array<{ test: RegExp; title: string; source: EvidenceSource }> = [
    { test: /^header\./, title: 'Notification detail', source: SAP_NOTIFICATION },
    {
        ...
        source: { label: 'SAP master data', kind: 'sap', detail: 'Material and work centre master data in SAP.' },
    },
    {
        test: /^inspections/,
        title: 'Measurement taken',
        source: { label: 'SAP QALS/QAMR', kind: 'sap', detail: 'Inspection lot and characteristic results in SAP QM.' },
    },
    {
        test: /^historicalInspectionLots/,
        title: 'Earlier measurement',
        source: { label: 'SAP QALS', kind: 'sap', detail: 'Past inspection lots for the same part and machine.' },
    },
    { test: /^actions/, title: 'Action on the notification', source: SAP_NOTIFICATION },
    {
        test: /^team/,
        title: 'Team member',
        source: { label: 'SAP partner roles', kind: 'sap', detail: 'Partner assigned on the notification.' },
    },
    {
        test: /^fmea/,
        title: 'Linked FMEA',
        source: { label: 'SAP DMS', kind: 'sap', detail: 'FMEA document linked to this case.' },
    },
    {
        test: /^isIsNot/,
    ...
    { test: /^rootCause/, title: 'Root cause on record', source: THIS_CASE },
    {
        ...
        source: {
            label: 'supported by SAP HANA',
            kind: 'sap',
            detail: 'Retrieved from past 8D cases via SAP HANA Knowledge Graph.',
        },
    },
    {
        test: /^independent/,
