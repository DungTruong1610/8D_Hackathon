...
> **Author:** Thanh (`feature/re-rank`, commit `aa604cb`) · **Saved here:** 2026-09-03 from `eval_plan.md`
>
> **Implementation status on `feat/graph-retrieval`** — steps 1-3 of §"Implementation order" are done:
> `rubrics.ts`, `compliance.ts`, `evalScoring.ts`, `judge.ts`, `db/schema/eval.cds`, a minimal
> `evalRepository.ts`, and `npm run eval:judge` to grade real reports. Steps 4-10 (batch `analyze`
> mode, advisor stages 1-2, Evaluation UI tab, online hook, calibration) are **not** started.
> See `docs/GRAPH-RETRIEVAL-AND-RERANK.md` for what the retrieval half of this branch already does.
